...
class WaveformDataset(Dataset):
    def __init__(self, dataset, limit=None, offset=0, sample_length=80000):
    ...
    def __getitem__(self, item):
        ...
        dir_name = os.path.basename(os.path.dirname(mixture_path))
        file_name = os.path.splitext(os.path.basename(mixture_path))[0]
        name = f"{dir_name}_{file_name}"

        _, mixture = scipy.io.wavfile.read(mixture_path) 
...
mixture_path = "Audio/source_separation/train_small\\0011\\mix_snr_-1.wav"
dir_name = os.path.basename(os.path.dirname(mixture_path))  # Extrait '0011'
file_name = os.path.splitext(os.path.basename(mixture_path))[0]  # Extrait 'mix_snr_-1'
name = f"{dir_name}_{file_name}"  # Combine les deux parties
print(name)
