"code adapted from : https://github.com/haoxiangsnr/Wave-U-Net-for-Speech-Enhancement/tree/master"
...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, item):
        mixture_path, clean_path, noise_path = self.dataset_list[item].split(" ")
        _, mixture = scipy.io.wavfile.read(os.path.abspath(mixture_path)) 
        _, clean = scipy.io.wavfile.read(os.path.abspath(clean_path)) 
        _, noise = scipy.io.wavfile.read(os.path.abspath(noise_path))

        name = os.path.splitext(os.path.basename(mixture_path))[0]
        # Resample mixture and clean to 16384 points
        mixture = mixture[:16384]
        clean = clean[:16384]
        noise = noise[:16384]

        return mixture.reshape(1, -1), clean.reshape(1, -1), noise.reshape(1, -1), name