...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        loss_total = 0.0
        for i, (mixture, clean, noise, name) in enumerate(self.train_dataloader):
            mixture = mixture.to(self.device)
            clean = clean.to(self.device)
            noise_sep = noise.to(self.device)

            self.optimizer.zero_grad()
            enhanced, noise = self.model(mixture)
            loss1 = self.loss(enhanced, clean)
            loss2 = self.loss(noise, noise_sep)
            loss = loss1 + loss2
            loss.backward()
            self.optimizer.step()
    ...
    def _validation_epoch(self, epoch):
        ...
        for i, (mixture, clean, noise,  name) in enumerate(self.validation_dataloader):
            ...
            for chunk in mixture_chunks:
                noised_chunks.append(self.model(chunk)[1].detach().cpu())
                enhanced_chunks.append(self.model(chunk)[0].detach().cpu())
